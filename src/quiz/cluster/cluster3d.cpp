...
void render3DTree(Node* node, pcl::visualization::PCLVisualizer::Ptr& viewer, Box window, int& iteration, uint depth=0)
{
	...
	if(node!=NULL)
	{
		...
		if(depth%3==0)
		{
			viewer->addLine(pcl::PointXYZ(node->point[0], window.y_min, 0),pcl::PointXYZ(node->point[0], window.y_max, 0),0,1,0,"line"+std::to_string(iteration));
		...
		if(depth%3==2)
		{
			viewer->addLine(pcl::PointXYZ(window.x_min, window.y_min, 0),pcl::PointXYZ(window.x_max, window.y_max, 0),0,0,1,"line"+std::to_string(iteration));
...
int main ()
{
	...
	// Create data
	std::vector<std::vector<float>> points = { {-6.2,7,1.0}, {-6.3,8.4,1.0}, {-5.2,7.1,1.0}, {-5.7,6.3,1.0},
												{7.2,6.1,-4.0}, {8.0,5.3,-4.0}, {7.2,7.1,-4.0},
												{0.2,-7.1,4.0}, {1.7,-6.9,4.0}, {-1.2,-7.2,4.0}, {2.2,-8.9,4.0} };
